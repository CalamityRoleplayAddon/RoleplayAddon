private void AI_067_FreakingPirates()
{
	...
	bool flag = Projectile.type == 393 || Projectile.type == 394 || Projectile.type == 395;
	...
	*/
	...
	if (Projectile.localAI[0] >= (float)Main.rand.Next(900, 1200))
	{
		...
		int num54 = Gore.NewGore(Projectile.Center + Vector2.UnitX * -Projectile.direction * 8f, Vector2.Zero, Main.rand.Next(580, 583));
		Main.gore[num54].velocity /= 2f;
		Main.gore[num54].velocity.Y = Math.Abs(Main.gore[num54].velocity.Y);
		Main.gore[num54].velocity.X = (0f - Math.Abs(Main.gore[num54].velocity.X)) * (float)Projectile.direction;
	}
}